...
- **LSTM 是 RNN 的变体，设计目标是解决传统 RNN 的梯度消失和梯度爆炸问题，特别适合处理长序列中的长期依赖关系**
...
- 总结
  - RNN 的隐藏状态像一个简单的 “记忆池”，无选择地存储所有信息
  - LSTM 的记忆单元像一个 “智能过滤器”，通过门控机制动态选择重要信息存储或丢弃
...
- 流程图表示（ \* 用来代替 $\odot$ 符号 ）
  ...
  graph TD
    ...
    G -->|提供 c̃ᵗ| H
    K -->|提供 cᵗ⁻¹| H
    D -->|提供 iᵗ| H[记忆单元 cᵗ = fᵗ * cᵗ⁻¹ + iᵗ * c̃ᵗ]
    E -->|提供 fᵗ| H
