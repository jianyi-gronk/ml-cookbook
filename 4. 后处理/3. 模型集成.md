...
- 使用扩展（ 同质模型的变体组合 ）
  - **相同模型，但不同参数组合**，如 决策树层数，决策树节点数 等，捕捉不同层次的特征之间关联
  - **相同模型，但不同 seed 组合**，避免不同 seed 对模型性能的扰动
...
- 核心思想和 Stacking 类似，但是不使用交叉验证，而是 **直接将训练集划分两部分，一部分训练基模型，一部分生成元模型的特征**，这样计算效率更高

## 8. Hill Climbing

- 核心思想
  - 通过在当前解的邻域内不断寻找更优解来逐步提升集成模型性能，即一个一个加集成模型，只保留能提升的
  - 是贪心算法，每次都向局部最优解移动，容易陷入局部最优
- 优化算法
  - 随机化重启爬山算法
    - 通过多次运行爬山算法，每次使用不同的初始解来尝试得到更好的全局最优解
  - 模拟退火算法
    - 在爬山算法的基础上引入随机性，允许以一定概率接受比当前解更差的解，从而有机会跳出局部最优，找到全局最优解
  - 遗传算法
    - 模拟自然界中的优胜劣汰过程，通过产生新一代的解并在其间进行筛选，来实现全局优化的过程
